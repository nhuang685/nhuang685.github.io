...
queues, we can enumerate all substrings covering the intersection of length
less than or equal to $k$. We can find the frequency of each substring using
a hash table and the queues using a doubly-linked list.
...
number of substrings is bounded by $\mathcal{O}(nk)$, the actual number of
substrings we add/remove is $\mathcal{O}(nk + ck^2)$, because we can remove
